# Микросервис для драйвера РК/ПКУ

## Содержание

- [Структура проекта](#структура-проекта)
- [Диаграммы C4](#диаграммы-c4)
    - [Диаграмма контекста](#диаграмма-контекста)
    - [Диаграмма контейнеров](#диаграмма-контейнеров)
    - [Диаграмма компонентов](#диаграмма-компонентов)
- [Зависимости](#зависимости)
    - [Зависимости для Linux](#зависимости-для-linux)
        - [Python](#python)
        - [pip](#pip)
        - [gRPC для Python](#grpc-для-python)
        - [CMake](#cmake)
        - [Make](#make)
        - [g++](#g) 
        - [gRPC для C++](#grpc-для-c)
- [Сборка](#сборка)
    - [Сборка в Linux](#сборка-в-linux)
- [Очистка](#очистка)
    - [Очистка в Linux](#очистка-в-linux)
- [Запуск](#запуск)
    - [Запуск в Linux](#запуск-в-linux)
- [Тестирование](#тестирование)
    - [Тестирование в Linux](#тестирование-в-linux)

## Структура проекта

```
.
├── c_1 # Диаграмма контекста
├── c_2 # Диаграмма контейнеров
├── c_3 # Диаграмма компонентов
├── src # Исходный код
│   ├── api # API gRPC
│   │   ├── clean.sh # Очистка кода gRPC
│   │   ├── generate.sh # Генерация кода gRPC
│   │   ├── pku_driver.proto # API драйвера
│   │   └── pku_service.proto # API микросервиса
│   ├── build.sh # Сборка драйвера
│   ├── clean.sh # Очистка драйвера
│   ├── CMakeLists.txt # Файл сборки драйвера
│   ├── configuration # Конфигурация
│   │   ├── driver_address.json # Адрес драйвера
│   │   └── pku_service_address.txt # Адрес микросервиса
│   ├── driver # Исходный код драйвера
│   ├── service # Исходный код микросервиса
│   └── tests # Тесты
└── use_case_diagram # Диаграмма вариантов использования
```

## Диаграмма вариантов использования

![use_case](use_case_diagram/use_case.svg)

## Диаграммы C4

### Диаграмма контекста

![c_1](c_1/c_1.svg)

### Диаграмма контейнеров

![c_2](c_2/c_2.svg)

### Диаграмма компонентов

![c_3](c_3/c_3.svg)

## Зависимости

### Зависимости для Linux

Для работы микросервиса достаточно установить следующие зависимости:

1. Python;
2. pip;
3. gRPC для Python.

Для сборки заглушки драйвера достаточно установить следующие зависимости:

1. CMake;
2. Make;
3. g++;
4. gRPC для C++.

#### Python

Инструменты разработки на Python можно установить с помощью пакетного менеджера.

```bash
sudo apt-get install python3
```

#### pip

Пакетный менеджер pip для Python можно установить с помощью пакетного менеджера.

```bash
sudo apt-get install python3-pip
```

#### gRPC для Python

Набор модулей gRPC можно установить с помощью пакетного менеджера Python.

```bash
python3 -m pip install grpcio
```

Генератор gRPC-кода protoc можно установить с помощью пакетного менеджера Python.

```bash
python3 -m pip install grpcio-tools
```

#### CMake

Систему сборки CMake можно установить с помощью пакетного менеджера.

```bash
sudo apt-get install cmake
```

#### Make

Систему сборки Make можно установить с помощью пакетного менеджера.

```bash
sudo apt-get install make
```

#### g++

Компилятор g++ можно установить с помощью пакетного менеджера.

```bash
sudo apt-get install g++
```

#### gRPC для C++

Библиотеки gRPC собираются из исходного кода.
Для этого нужно клонировать официальный репозиторий.

```bash
git clone --recurse-submodules -b v1.82.0 --depth 1 --shallow-submodules https://github.com/grpc/grpc
```

В переменную окружения ```MY_INSTALL_DIR``` нужно поместить директорию, в которую будут устанавливаться библиотеки gRPC.

Для сборки выполняются следующие команды.

```bash
cd grpc
mkdir -p cmake/build
pushd cmake/build
cmake -DgRPC_INSTALL=ON -DgRPC_BUILD_TESTS=OFF -DCMAKE_CXX_STANDARD=17 -DCMAKE_INSTALL_PREFIX=$MY_INSTALL_DIR ../..
make -j 4
make install
popd
```

В переменную окружения ```PATH``` нужно поместить директорию ```$MY_INSTALL_DIR/bin``` с бинарными файлами gRPC.

В переменную окружения ```PKG_CONFIG_PATH``` нужно поместить директорию ```MY_INSTALL_DIR/lib/pkgconfig``` с конфигурационными файлами библиотек.

## Сборка

### Сборка в Linux

Генерация кода gRPC выполняется с помощью скрипта ```src/api/generate.sh```.

Cборка без генерации кода gRPC выполняется с помощью скрипта ```src/build.sh```.

## Очистка

### Очистка в Linux

Очистка кода gRPC выполняется с помощью скрипта ```src/api/clean.sh```.

Очистка результатов сборки, не включая код gRPC, выполняется с помощью скрипта ```src/clean.sh```.

## Запуск

### Запуск в Linux

Драйвер запускается с помощью скрипта ```src/driver/run.sh```.

Микросервис запускается с помощью скрипта ```src/service/run.sh```.

## Тестирование

### Тестирование в Linux

#### Модульное тестирование

Модульные тесты микросервиса запускаются с помощью скрипта ```src/service/run_tests.sh```.

#### Интеграционное тестирование

Для запуска интеграционных тестов нужно запустить драйвер, микросервис и сами тесты в следующем порядке:

1. ```src/tests/run_driver.sh```;
2. ```src/tests/run_service.sh```;
3. ```src/tests/run_tests.sh```.
