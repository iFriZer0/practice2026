...
class IntegrationTests(unittest.TestCase):
    ...
    def test_check_connection(self) -> None:
        request: pku_service_pb2.CommandRequest = pku_service_pb2.CommandRequest(
            command_id=self.Commands.CHECK_CONNECTION.value, command_param=""
        )
    ...
    def test_get_hardware_status(self) -> None:
        request: pku_service_pb2.CommandRequest = pku_service_pb2.CommandRequest(
            command_id=self.Commands.GET_HARDWARE_STATUS.value,
            command_param=""
        )
        response: pku_service_pb2.CommandResponse = self.stub.SendCommand(request)
        self.assertTrue(response.success)
    ...
    def test_read_pku(self) -> None:
        request: pku_service_pb2.CommandRequest = pku_service_pb2.CommandRequest(
            command_id=self.Commands.READ_PKU.value,
            command_param=self.DELIMITER.join(["1", "2", "4", "op"])
        )
        response: pku_service_pb2.CommandResponse = self.stub.SendCommand(request)
        self.assertTrue(response.success)
    ...
    def test_set_pku_mode(self) -> None:
        request: pku_service_pb2.CommandRequest = pku_service_pb2.CommandRequest(
            command_id=self.Commands.SET_PKU_MODE.value,
            command_param=self.DELIMITER.join(["1", "0", "op"])
        )
        response: pku_service_pb2.CommandResponse = self.stub.SendCommand(request)
        self.assertTrue(response.success)
        self.assertEqual(response.result_text, "")
    ...
    def test_send_rk_by_index(self) -> None:
        request: pku_service_pb2.CommandRequest = pku_service_pb2.CommandRequest(
            command_id=self.Commands.SEND_RK_BY_INDEX.value,
            command_param=self.DELIMITER.join(["1", "100", "op"])
        )
        response: pku_service_pb2.CommandResponse = self.stub.SendCommand(request)
        self.assertTrue(response.success)
        self.assertEqual(response.result_text, "")
    ...
    def test_get_version(self) -> None:
        request: pku_service_pb2.CommandRequest = pku_service_pb2.CommandRequest(
            command_id=self.Commands.GET_VERSION.value,
            command_param=""
        )
        response: pku_service_pb2.CommandResponse = self.stub.SendCommand(request)
        self.assertTrue(response.success)
